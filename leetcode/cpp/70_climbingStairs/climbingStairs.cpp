...
class Solution {
   public:
    ...
    (3) 利用之前计算的结果一步步递推。初始化时，令dp[0] = dp[1] =
    1,然后递推dp[n] = dp[n - 1] + dp[n - 2]即可 (4) 优化空间复杂度，使用int
    dp[3] = {1,1,1}; dp[2] = dp[1] +  dp[0]; dp[0] = dp[1]; dp[1] = dp[2];
    迭代更新，空间复杂度为O(1)
  ...
  */
    ...
    int climbStairs2(int n) {
        //节省内存, 定义3个变量即可，或一个大小为3的数组
        // dp[0]=1 就是为了满足dp[2]=dp[1]+dp[0]
        vector<int> dp = {1, 1, 1};
        for (int i = 2; i <= n; ++i) {
            dp[2] = dp[1] + dp[0];
            dp[0] = dp[1];
